...
import (
	...
	"github.com/Sandwichzzy/relayer-node/common/bigint"
	"github.com/Sandwichzzy/relayer-node/common/tasks"
	"github.com/Sandwichzzy/relayer-node/database"
	common2 "github.com/Sandwichzzy/relayer-node/database/common"
...
type Synchronizer struct {
	...
	relayerRegistry  *metrics.SyncerMetrics
...
func NewSynchronizer(cfg *Config, db *database.DB, client node.EthClient, relayerRegistry *metrics.SyncerMetrics, shutdown context.CancelCauseFunc) (*Synchronizer, error) {
	//从数据库查询最新的区块头
	...
	return &Synchronizer{
		...
		headerTraversal:  node.NewHeaderTraversal(client, fromHeader, cfg.ConfirmationDepth, uint(chainIdInt)), //创建 HeaderTraversal 实例用于遍历区块
		ethClient:        client,
	...
	syncer.tasks.Go(func() error {
		for range tickerSyncer.C {
			if len(syncer.headers) > 0 {
				...
				syncer.relayerRegistry.RecordChainBlockHeight(syncer.chainId, latestHeader.Number)
			}
...
func (syncer *Synchronizer) processBatch(headers []types.Header) error {
	...
	for i := range headers {
		header := headers[i]
		headerMap[header.Hash()] = &header //构建区块头映射
	}
	...
	//将日志事件转换为 ChainContractEvent 格式
